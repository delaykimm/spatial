...
스크립트는 역할별로 5개 폴더에 나뉘어 있음. 서로 다른 폴더의 스크립트를 import해도(예: `chain_hop/`가 `core/`의 `axis_pipeline`을 import) 각 파일 상단의 `sys.path` 부트스트랩이 5개 폴더를 전부 경로에 추가하므로, **어느 디렉토리에서 실행하든** `python core/axis_pipeline.py ...`처럼 리포지토리 루트 기준 상대경로로 스크립트를 지정하기만 하면 됨 (실행 시 현재 작업 디렉토리는 상관없음 — `__file__` 기준으로 루트를 계산함).

| 폴더 | 내용 |
|---|---|
| `core/` | 모델 로딩·hidden-state 추출 엔진, triplet/sameaxis 데이터셋 생성 — 다른 모든 폴더가 이 두 파일을 import함 |
| `analysis/` | compositionality/alignment/readout 비교, 데이터셋 간 비교 |
| `steering/` | 축 벡터를 hidden state에 주입하는 인과성(causal) 검증 |
| `chain_hop/` | N-hop 랜덤워크 체인 (렌더링 + CLEVR 실사진 버전) |
| `multihop_referring/` | 중첩 지시표현(nested referring expression)으로 진짜 multi-hop 추론을 강제하는 버전 |

`data/`, `results/`, `config.yaml`, `README.md`, `requirements.txt`, `reproduce_datasets/`는 최상위에 그대로 있음.
...
| `core/axis_pipeline.py` | What'sUp/SpatialTunnel/Aug1/Aug2 4개 데이터셋에서 방향 벡터(축) 추출 |
| `core/triplet_pipeline.py` | clevr/triplet3ax(cross-axis)/sameaxis 3종 triplet 데이터셋 생성 + hidden state 추출 (다른 대부분의 스크립트가 이걸 먼저 필요로 함) |
| `steering/axis_steering.py` | 그 축 벡터를 모델 hidden state에 주입해서 실제로 답이 바뀌는지 테스트 |
| `analysis/cross_dataset_analysis.py` | 4개 외부 데이터셋끼리 축 벡터/steering 결과 비교 |
| `analysis/cross_axis_analysis.py` | compositionality: r_AB + r_BC ≈ r_AC 인지 (자기 표상 내부 일관성) |
| `analysis/cross_axis_alignment.py` | alignment: 모델 벡터가 실제 물리적 방향과 일치하는지 |
| `analysis/cross_axis_readout_vqa.py` | sign-match readout(기하학적) vs VQA(직접 질문) 정확도 비교 |
| `analysis/random_pair_alignment.py` | CLEVR 임의 오브젝트 쌍(구조 없음)에 대한 alignment 애드혹 점검 |
| `analysis/sameaxis_4way_readout_vqa.py` | sameaxis에서 A/B/C/D 4개 중 극값(제일 왼쪽 등) 찾기 readout vs VQA |
| `steering/diffpair_steering.py` | 같은 이미지, 다른 pair 벡터를 주입해도 causal하게 작동하는지 |
| `steering/diffimage_steering.py` | 다른 이미지, 같은 관계(pair) 벡터를 주입해도 causal하게 작동하는지 |
| `chain_hop/chain_hop_pipeline.py` | N-hop 랜덤워크 체인(A→B→C→...→Z, 한 축 위, 각 hop 부호 랜덤) **렌더링** + hidden state 추출 |
| `chain_hop/clevr_chain_pipeline.py` | 위와 같은 체인을 **실제 CLEVR 사진에서** 장면당 임의 N개 오브젝트/랜덤 순서로 구성 (렌더링 없음, horizontal/closefar 2축만) + hidden state 추출 |
| `chain_hop/chain_hop_readout_vqa.py` | hop 수(2~6)별로 latent computation(Σc_i,i+1 축 투영 sign) vs MLLM generation(VQA) 정확도 비교, hop-accuracy 플롯 생성. `--source {synthetic,clevr}`로 위 두 백엔드 중 선택 |
| `multihop_referring/multihop_referring_pipeline.py` | 중첩 지시표현("...의 오른쪽의 왼쪽의 ... 오브젝트")으로 Z를 실제로 지목하게 하는 체인 **렌더링** + hidden state 추출 |
| `multihop_referring/clevr_multihop_referring_pipeline.py` | 위와 같은 체인을 **실제 CLEVR 사진에서** 구성 (렌더링 없음) |
| `multihop_referring/multihop_referring_readout_vqa.py` | latent computation vs 진짜 multi-hop MLLM generation(Z vs 한 hop 모자란 Y 판별) 정확도 비교. `--source {synthetic,clevr}` |
...
```
python core/axis_pipeline.py --dataset whatsup --model qwen3vl      # spatialtunnel/aug1/aug2도 동일하게 반복
python steering/axis_steering.py --dataset whatsup                  # 동일하게 4개 반복
```

**2) triplet 데이터셋 추출** (clevr/triplet3ax/sameaxis — 이 아래 대부분의 스크립트가 필요로 함)
```
python core/triplet_pipeline.py --dataset clevr --model qwen3vl
python core/triplet_pipeline.py --dataset triplet3ax --model qwen3vl
python core/triplet_pipeline.py --dataset sameaxis --model qwen3vl
```

**3) triplet 기반 핵심 분석/steering** (2번만 있으면 됨, 순서 무관)
```
python analysis/cross_axis_alignment.py --dataset clevr --model qwen3vl      # readout_vqa가 이 결과를 씀 → 먼저 실행
python analysis/cross_axis_readout_vqa.py --dataset clevr --model qwen3vl
python analysis/cross_axis_analysis.py --dataset clevr --model qwen3vl
...
- **외부 데이터셋 4개끼리 서로 얼마나 비슷한 축을 갖는지 비교**: 1번의 `axis_pipeline.py`/`axis_steering.py` 결과가 있어야 함
  ```
  python analysis/cross_dataset_analysis.py
  ```
- **compositionality + alignment + magnitude fidelity를 한 그래프로**: 3번의 `cross_axis_alignment.py` 결과가 있어야 함
  ```
  cd analysis
  python3 -c "import cross_axis_alignment as cag; cag.draw_composition_grounding_joint_plot('clevr')"
  ```
...
- **synthetic** (`chain_hop_pipeline.py`): 렌더링으로 새로 생성, 한 축 위 랜덤워크 체인(hop=2~6), 3축(horizontal/vertical/closefar) 전부 지원
  ```
  python chain_hop/chain_hop_pipeline.py --model qwen3vl
...
**6) 진짜 multi-hop 추론 (중첩 지시표현)** (5번과 독립, 두 백엔드 중 택1)
...
- **clevr**:
  ```
  python multihop_referring/clevr_multihop_referring_pipeline.py --clevr-val-dir /path/to/CLEVR_v1.0/images/val --model qwen3vl
